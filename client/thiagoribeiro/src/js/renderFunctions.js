...
function makeFavoriteElement(){
  ...
  favoriteElement.appendChild(SVGElement);
  ...
  favoriteElement.addEventListener("click", element => {
    ...
    if(element.currentTarget.classList.contains("favorite")){
      let pathElement = element.currentTarget.firstChild.firstChild;
      pathElement.setAttribute("d", clickedPath);
    } else {
      let pathElement = element.currentTarget.firstChild.firstChild;
      pathElement.setAttribute("d", defaultPath);
    }
  });
...
function newsTemplate(newsList){
  ...
  element.appendChild(favoriteElement);
